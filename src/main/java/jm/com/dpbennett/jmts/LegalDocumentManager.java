...
public class LegalDocumentManager implements Serializable {
    ...
    //@PersistenceUnit(unitName = "LegalDepartmentPortalPU")
...
//        jm.saveBusinessEntity(em, user);
    ...
    public void createNewLegalDocument(ActionEvent action) {
        selectedDocumentId = currentDocument.getId();
        currentDocument = createNewLegalDocument(getEntityManager(), user);
    ...
    public List<LegalDocument> getDocumentSearchByTypeResultList() {
        ...
        if (selectedDocument != null) {
            return LegalDocument.findLegalDocumentsByDateSearchField(em,
                    dateSearchField, "By type", selectedDocument.getType().getName(), 
                    startDate, endDate);
        } else {
            return new ArrayList<>();
        }
    ...
    public void login(ActionEvent actionEvent) {
        ...
        if ((getUsername() != null) && (getPassword() != null)) {
            if (validateUser(getUsername(), getPassword())) {
//            if (true) { // tk NB: REMOVE!!!!
    ...
    // tk use WebApp version when this app is merged with BSJDB app
